...
const convertStyle = <T,>(
  propsStyle: StyleProp<T> | undefined,
  rnStyle: Style,
  units: Units,
): StyleProp<T> => {
  ...
  Object.keys(rnStyle).forEach(key => {
    ...
    if (key === 'transform') {
      ...
      convertedStyle.transform = (rnStyle.transform!.map(transformation => {
        ...
        (Object.keys(transformation) as Array<
          keyof typeof transformation
        >).forEach(
          k => (result[k] = convertValue(k, transformation[k]!, units)),
        );
        return result;
      ...
      convertedStyle.shadowOffset = {
        ...
        width: convertValue(
          key,
          rnStyle.shadowOffset!.width || '0',
          units,
        ) as number,
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        height: convertValue(
          key,
          rnStyle.shadowOffset!.height || '0',
          units,
        ) as number,
      };
    } else {
  ...
  return (propsStyle
    ? [convertedStyle, propsStyle]
    : convertedStyle) as StyleProp<T>;
};
