...
function findNumbers(value: string) {
  ...
  value
    .split(/\s+/gm)
    .forEach(val => result[isNumber(val) ? 'numbers' : 'nonNumbers'].push(val));
  return result;
}
...
function read3D(prefix: 'rotate', value: string): Transform[] {
  ...
  const transformt = [];
  if (x) transformt.push({ [`${prefix}X`]: x });
  if (y) transformt.push({ [`${prefix}Y`]: y });
  if (z) transformt.push({ [`${prefix}Z`]: z });
  return transformt;
}
...
export function transform(value: string) {
  ...
  const transform = [...matchAll(value, regexp)].reduce((acc, val) => {
    const operation = val[1];
    const values = val[2].trim();
    if (['translate', 'scale', 'skew'].includes(operation))
      return acc.concat(
        read2D(operation as 'translate' | 'scale' | 'skew', values),
      );
    if (operation === 'rotate3d') return acc.concat(read3D('rotate', values));
...
export function animation(value: string) {
  ...
  const animation = [...matchAll(value, regexp)].reduce((acc, val) => {
    const operation = val[1];
    const values = val[2].trim();
    if (['translate', 'scale', 'skew'].includes(operation))
      return acc.concat(
...
export function font(value: string) {
  ...
  for (let i = 0; i < nonNumbers.length; i++) {
    ...
    else if (val === 'normal') continue;
    // can be both fontStyle or fontWeight, but as it is the default we can just ignore.
    else if (
      [
        'small-caps',
        'oldstyle-nums',
...
export function sideValue(
  prefixKey: 'padding' | 'margin' | 'border',
  value: string,
  postFix: 'Width' | '' = '',
): { [x: string]: string } {
  if (value === 'none') return sideValue(prefixKey, '0', postFix);
  const [top, right = top, bottom = top, left = right] = findNumbers(
    value,
  ).numbers;
  return {
