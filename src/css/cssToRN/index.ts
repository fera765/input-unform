...
function stripSpaces(string: string) {
  return string.replace(/(calc|max|min)\(.*?\)/gm, res =>
    res.replace(/\s/g, ''),
  );
}
...
function cssToStyle(css: string) {
  const result: Style = {};
  // Find hover
  const cssWithoutHover = css.replace(/&:hover\s*{(.*?)}/gims, res => {
    result.hover = cssChunkToStyle(
      res.substring(0, res.length - 1).replace(/&:hover\s*{/ims, ''),
    );
    return '';
  });
...
function cssChunkToStyle(css: string) {
  const result: Style = {};
  css.split(/\s*;\s*/gm).forEach((entry: string) => {
    ...
    switch (key) {
      ...
      case 'transform':
        ...
        break;
      case 'animation':
        Object.assign(result, animation(value));
        break;
      case 'boxShadow':
      case 'textShadow':
        ...
        Object.assign(
          result,
          shadow(key === 'boxShadow' ? 'shadow' : key, value),
        );
        break;
