...
function buildCSSString<T extends { rnCSS?: string }>(
  chunks: TemplateStringsArray,
  functs: (Primitive | Functs<T>)[],
  props: T,
) {
  let computedString = chunks
    .map((chunk, i) => [
      chunk,
      functs[i] instanceof Function
        ? (functs[i] as Functs<T>)(props)
        : functs[i],
    ])
    .flat()
    .join('');
...
const styled = <Props,>(Component: React.ComponentType<Props>) => {
  const styledComponent = <S,>(
    chunks: TemplateStringsArray,
    ...functs: (Primitive | Functs<S & Props>)[]
  ) => {
    const ForwardRefComponent = React.forwardRef<
      React.ComponentType<S & Props & OptionalProps>,
      S & Props & OptionalProps
    >((props: S & Props & OptionalProps, ref) => {
      const units = React.useRef<Units>({
        em: 16,
        vw: 1,
        vh: 1,
        vmin: 1,
      ...
      // Handle hover
      const { onMouseEnter, onMouseLeave, style: hoverStyle } = useHover(
        rnStyle,
        props.onMouseEnter,
        props.onMouseLeave,
      );
      ...
      // Handle em units
      const { em } = useFontSize(
        finalStyle.current.fontSize,
        units.current.rem,
      );
      ...
      if (
        needsLayout &&
        (units.current.width !== width || units.current.height !== height)
      ) {
      ...
      const { vw, vh, vmin, vmax, width: wdt, height: hgt } = useScreenSize();
      if (
        needsScreenSize &&
        (units.current.vw !== vw ||
          units.current.vh !== vh ||
          units.current.vmin !== vmin ||
          units.current.vmax !== vmax)
      ) {
        units.current = { ...units.current, vw, vh, vmin, vmax };
      }

      const styleConvertedFromCSS = React.useMemo(
        () => convertStyle(props.style, finalStyle.current, units.current),
        [props.style],
      );
      const zIndex = useZIndex(
        StyleSheet.flatten(styleConvertedFromCSS).zIndex,
      );
      const style: StyleProp<any> = React.useMemo(
        () =>
          zIndex ? [styleConvertedFromCSS, { zIndex }] : styleConvertedFromCSS,
        [styleConvertedFromCSS, zIndex],
      );
      ...
      if (em !== currentFontSize) {
        return (
          <FontSizeContext.Provider value={em}>
    ...
    return ForwardRefComponent as React.ForwardRefExoticComponent<
      Props & S & OptionalProps & { ref?: React.Ref<any> }
    >;
  };

  // provide styled(Comp).attrs({} | () => {}) feature
  styledComponent.attrs = <S,>(
    opts: Partial<S & Props> | ((props: S & Props) => Partial<S & Props>),
  ) => (
    chunks: TemplateStringsArray,
    ...functs: (Primitive | Functs<S & Props>)[]
  ) => {
      const ComponentWithAttrs = styledComponent(chunks, ...functs);
      const ForwardRefComponent = React.forwardRef<typeof Component, S & Props>(
        (props: Props & S, ref) => {
...
export const styledFlatList = <S,>(
  chunks: TemplateStringsArray,
  ...functs: (Primitive | Functs<S>)[]
) => <Type,>(props: S & FlatListProps<Type> & OptionalProps) =>
    invoke(styled<FlatListProps<Type>>(FlatList)(chunks, ...functs), props);
styledFlatList.attrs = <S,>(
  opts:
    | Partial<S & FlatListProps<any>>
    | ((props: S & FlatListProps<any>) => Partial<S & FlatListProps<any>>),
) => (
  chunks: TemplateStringsArray,
  ...functs: (Primitive | Functs<S & FlatListProps<any>>)[]
) => <Props,>(componentProps: S & FlatListProps<Props> & OptionalProps) =>
      invoke(
        styled<FlatListProps<Props>>(FlatList).attrs<S>(opts)(chunks, ...functs),
        componentProps,
      );

export const styledSectionList = <S,>(
  chunks: TemplateStringsArray,
  ...functs: (Primitive | Functs<S>)[]
) => <Type,>(props: S & SectionListProps<Type> & OptionalProps) =>
    invoke(styled<SectionListProps<Type>>(SectionList)(chunks, ...functs), props);
styledSectionList.attrs = <S,>(
  opts:
    | Partial<S & SectionListProps<any>>
    | ((
      props: S & SectionListProps<any>,
    ) => Partial<S & SectionListProps<any>>),
) => (
  chunks: TemplateStringsArray,
  ...functs: (Primitive | Functs<S & SectionListProps<any>>)[]
) => <Props,>(componentProps: S & SectionListProps<Props> & OptionalProps) =>
      invoke(
        styled<SectionListProps<any>>(SectionList).attrs(opts)(chunks, ...functs),
        componentProps,
      );
export const styledVirtualizedList = <S,>(
  chunks: TemplateStringsArray,
  ...functs: (Primitive | Functs<S>)[]
) => <Type,>(props: S & VirtualizedListProps<Type> & OptionalProps) =>
    invoke(
      styled<VirtualizedListProps<Type>>(VirtualizedList)(chunks, ...functs),
      props,
    );
styledVirtualizedList.attrs = <S,>(
  opts:
    | Partial<S & VirtualizedListProps<any>>
    | ((
      props: S & VirtualizedListProps<any>,
    ) => Partial<S & VirtualizedListProps<any>>),
) => (
  chunks: TemplateStringsArray,
  ...functs: (Primitive | Functs<S & VirtualizedListProps<any>>)[]
) => <Props,>(
  componentProps: S & VirtualizedListProps<Props> & OptionalProps,
    ) =>
      invoke(
        styled<VirtualizedListProps<Props>>(VirtualizedList).attrs(opts)(
          chunks,
          ...functs,
        ),
        componentProps,
